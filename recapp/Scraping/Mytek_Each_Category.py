...
main_url = "https://www.mytek.tn/informatique.html"

with Session() as session:
    # Get category links
    r = session.get(main_url)
    soup = BeautifulSoup(r.content, 'lxml')
    category_links = [a['href'] for a in soup.select('a.mkcache')]
    category_links = [link for link in category_links if not link.startswith("hhttps://")]

    for lien in category_links:
        r = session.get(lien)
...
df.to_csv('C:/Users/zagho/recproject/recapp/Scraping/MyTek_INFORMATIQUE.csv', index=False)